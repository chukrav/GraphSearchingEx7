...
namespace GraphSearching
{
    ...
    class Program
    {
        ...
        static void Main(string[] args) {
            //Graph<int> graph = BuildGraph_ex7();
            //testGraphEx7(graph);
        ...
        static void testLinesSquare(Graph<int> graph, int start, int finish) {

            // breadth-first search
            Console.WriteLine("-------- testLinesSquare --------");
            Console.WriteLine("Breadth-First Search");
            Console.WriteLine("------------------");
            Console.WriteLine("Path from " + start + " to " + finish + ": " +
                Search(start, finish, graph, SearchType.BreadthFirst));
            ...
            Console.WriteLine("------------------");
            Console.WriteLine("Path from " + start + " to " + finish + ": " +
                Search(start, finish, graph, SearchType.DepthFirst));

            Console.WriteLine();

        }

        static void testGraphEx7(Graph<int> graph) {
        ...
        /// </summary>
        /// <returns></returns>
        static Graph<int> BuildGraph_ex7()
        {
            ...
            graph.AddEdge(4, 11, 22);
            graph.AddEdge(4, 42, 84);
            graph.AddEdge(5, 11, 22);
            graph.AddEdge(5, 12, 24);
            graph.AddEdge(5, 42, 84);
            graph.AddEdge(7, 10, 20);
            graph.AddEdge(7, 11, 22);
            graph.AddEdge(10, 11, 22);
            graph.AddEdge(11, 42, 84);
            graph.AddEdge(12, 42, 84);
            ...
            Graph<int> graph, SearchType searchType)
        {
            ...
            else
            {
                ...
                pathNodes.Add(startNode, new PathNodeInfo<int>(null,0));
            ...
            Dictionary<GraphNode<int>, PathNodeInfo<int>> pathNodes)
        {
            ...
            int pathCost = pathNodes[endNode].Cost;
            //int pathCost = previous.GetEdgeCost(endNode);
            while (previous != null)
            {
                path.AddFirst(previous);
                pathCost += pathNodes[previous].Cost;
               // pathCost += pathNodes[previous].Previous.GetEdgeCost(previous);
            ...
            pathString.Append(" Cost: " + pathCost);
